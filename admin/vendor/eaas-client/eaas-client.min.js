...
c.driveId=a.driveId;c.pollState()},function(a){c._onError($.parseJSON(a.responseText))})};this.getScreenshotUrl=function(){return d+g("/components/{0}/screenshot",c.componentId)};this.getPrintUrl=function(){return d+g("/components/{0}/print",c.componentId)};this.stopEnvironment=function(){"undefined"!==typeof this.guac&&this.guac.disconnect();$.ajax({type:"GET",url:d+g("/components/{0}/stop",c.componentId),async:!1});$(f).empty()};this.clearTimer=function(){clearInterval(this.keepaliveIntervalId)};
this.release=function(){this.stopEnvironment();this.clearTimer()};this.changeMedia=function(a,b){$.ajax({type:"POST",url:d+g("/components/{0}/changeMedia",c.componentId),data:JSON.stringify(a),contentType:"application/json"}).then(function(a,c,d){b(a,c)})};var t=function(a,b){function c(a){return window.location.getParameter(a)}function d(a,b){a=window.location.getParameter(a);return null==a?b:0<=["true","on","1"].indexOf(String(a).toLowerCase())}window.location.getParameter||(window.location.getParameter=
function(a){if(!this.queryStringParams){for(var b={},c,d=/\+/g,g=/([^&=]+)=?([^&]*)/g,f=window.location.search.substring(1);c=g.exec(f);)b[decodeURIComponent(c[1].replace(d," "))]=decodeURIComponent(c[2].replace(d," "));this.queryStringParams=b}return this.queryStringParams[a]});window.oncontextmenu=function(a){return!1};$(document).ready(function(){var g=c("username")||null,k=c("password")||null,f=c("sound")||"true",h=c("audio_codec")||"opus+mka",l=c("encoding")||null,m=c("action")||"connect";c("submit");
var n=a.substring(7,a.indexOf("8080")-1),q=a.substring(a.indexOf("8080")),r=c("encryption")||null,p=c("key")||null,w=c("keyboard_layout")||null,x=c("start"),t=c("exit_with_children")||"",z=c("exit_with_client")||"",A=d("sharing",!1),B=d("video",!1),C=d("mediasource_video",!1),D=d("normal_fullscreen",!1),E=d("remote_logging",!0),F=d("debug",!1),G=d("insecure",!1),H=d("ignore_audio_blacklist",!1),y=d("clipboard",!0),e=new XpraClient("emulator-container");e.debug=F;e.remote_logging=E;e.sharing=A;e.insecure=
G;e.clipboard_enabled=y;B?(e.supported_encodings.push("h264"),C&&e.supported_encodings.push("vp8+webm","h264+mp4","mpeg4+mp4")):l&&"auto"!==l&&e.enable_encoding(l);m&&"connect"!=m&&(sns={mode:m},x&&(sns.start=[x]),t&&(sns["exit-with-children"]=!0),z&&(sns["exit-with-client"]=!0),e.start_new_session=sns);D&&(e.normal_fullscreen_mode=!0);f&&(e.audio_enabled=!0,console.log("sound enabled, audio codec string: "+h),h&&0<h.indexOf(":")&&(f=h.split(":"),e.audio_framework=f[0],e.audio_codec=f[1]),e.audio_mediasource_enabled=
d("mediasource",!0),e.audio_aurora_enabled=d("aurora",!0));w&&(e.keyboard_layout=w);g&&(e.username=g);k&&(e.authentication_key=k);r&&(e.encryption=r,p&&(e.encryption_key=p));e.init(H);g="https:"==document.location.protocol;XpraClient.prototype._do_connect=function(a){this.protocol=a&&!XPRA_CLIENT_FORCE_NO_WORKER?new XpraProtocolWorkerHost:new XpraProtocol;this.protocol.set_packet_handler(this._route_packet,this);a="ws://";this.ssl&&(a="wss://");a+=this.host;a+=":"+this.port;this.protocol.open=function(a){var c=
this;this.worker=new Worker(b+"/js/Protocol.js");this.worker.addEventListener("message",function(b){var d=b.data;switch(d.c){case "r":c.worker.postMessage({c:"o",u:a});break;case "p":c.packet_handler&&c.packet_handler(d.p,c.packet_ctx);break;case "l":console.log(d.t);break;default:console.error("got unknown command from worker"),console.error(b.data)}},!1)};this.protocol.open(a);var c=this;this.hello_timer=setTimeout(function(){c.disconnect_reason="Did not receive hello before timeout reached, not an Xpra server?";
c.close()},this.HELLO_TIMEOUT)};XpraClient.prototype._get_desktop_size=function(){return[xpraShapes.xpraWidth,xpraShapes.xpraHeight]};XpraClient.prototype._get_DPI=function(){return xpraShapes.xpraDPI};e.connect=XpraClient.prototype.connect=function(a,c,d){console.log("connecting to xpra server "+a+":"+c+" with ssl: "+d);this.host=a;this.port=c;this.ssl=d;if(!this.encryption||this.encryption_key&&""!=this.encryption_key)if(window.Worker){console.log("we have webworker support");var e=this;a=new Worker(b+
"/js/lib/wsworker_check.js");a.addEventListener("message",function(a){switch(a.data.result){case !0:console.log("we can use websocket in webworker");e._do_connect(!0);break;case !1:console.log("we can't use websocket in webworker, won't use webworkers");e._do_connect(!1);break;default:console.log("client got unknown message from worker"),e._do_connect(!1)}},!1);a.postMessage({cmd:"check"})}else console.log("no webworker support at all.");else this.callback_close("no key specified for encryption")};
e._new_window=function(a,b,c,d,e,g,f,h){var u=document.createElement("div");u.id=String(a);var k=document.createElement("canvas");u.appendChild(k);document.getElementById("emulator-container").appendChild(u);k.width=d;k.height=e;b=new XpraWindow(this,k,a,b,c,d,e,g,f,h,this._window_geometry_changed,this._window_mouse_move,this._window_mouse_click,this._window_set_focus,this._window_closed);b.debug=this.debug;this.id_to_window[a]=b;f||(this.normal_fullscreen_mode&&"NORMAL"==b.windowtype&&(b.undecorate(),
...
c);var d=0;"which"in a?d=Math.max(0,a.which):"button"in a&&(d=Math.max(0,a.button)+1);a=$("#emulator-container").offset();b-=a.left;c-=a.top;return{x:b,y:c,button:d}};e.connect(n,q,g);y&&(n=$("#pasteboard"),n.on("paste",function(a){a=(a.originalEvent||a).clipboardData.getData("text/plain");e.send_clipboard_token(unescape(encodeURIComponent(a)));return!1}),n.on("copy",function(a){a=e.get_clipboard_buffer();$("#pasteboard").text(decodeURIComponent(escape(a)));$("#pasteboard").select();e.clipboard_pending=
...
!1))}))})};this.startEnvironmentWithInternet=function(a,b,c){$.ajax({type:"POST",url:d+"/components",data:JSON.stringify({environment:a,keyboardLayout:b,keyboardModel:c}),contentType:"application/json"}).done(function(a){this.tmpdata=a;$.ajax({type:"POST",url:d+"/networks",data:JSON.stringify({components:[{componentId:a.id}],hasInternet:!0}),contentType:"application/json"}).done(function(a){this.pollState(this.tmpdata.controlUrl.replace(/([^:])(\/\/+)/g,"$1/"),this.tmpdata.id)}.bind(this)).fail(function(a,
